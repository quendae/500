...
test('menu can resume an in-memory single-player game',async({page})=>{await page.goto('/');await page.click('#singleButton');await expect(page.locator('#hand .card').first()).toBeVisible();await page.click('#menuButton');await expect(page.locator('#mainMenu')).toBeVisible();await expect(page.locator('#continueButton')).toBeVisible();await expect(page.locator('#continueInfo')).toContainText('Bieżąca gra');await page.click('#continueButton');await expect(page.locator('#mainMenu')).toBeHidden();await expect(page.locator('#modeBadge')).toHaveText('SINGLE');await expect(page.locator('#hand .card').first()).toBeVisible();});
